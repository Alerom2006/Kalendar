...
async function init() {
    console.log(`[${widgetInstanceId}] Виджет "Календарь заказов" инициализирован`);
    ...
    async function getAccessToken() {
      ...
      return settings.access_token || localStorage.getItem(`amo_access_token_${widgetInstanceId}`) || null;
    }

    async function renderWidget() {
        const widgetContainer = document.getElementById(targetElementId);
        if (!widgetContainer) {
            console.error(`[${widgetInstanceId}] Не найден элемент для встраивания виджета.`);
            return;
        }
        ...
        widgetContainer.innerHTML = `
            <div class="container">
                ...
                <div id="auth-section" class="mt-3" style="display: ${accessToken ? 'none' : 'block'};">
        ...
        async function fetchDeals(year, month) {
          if (!accessToken) {
            console.warn(`[${widgetInstanceId}] Нет Access Token, пропуск запроса к API.`);
            return {};
          }
            ...
            try {
                const response = await fetch(`${API_URL}leads?filter[date_modified][from]=${startDate}&filter[date_modified][to]=${endDate}`, { // Удалена фильтрация по статусу
                    headers: {
                ...
                if (!response.ok) {
                    if (response.status === 401) { // Unauthorized - токен истек
                        console.warn(`[${widgetInstanceId}] Access Token истек, попытка обновить`);
                        await refreshToken();
        ...
        function processDeals(deals) {
            ...
            deals.forEach(deal => {
                ...
                if (deal.custom_fields_values) {
                    const orderDateField = deal.custom_fields_values.find(field => field.field_id == DEAL_DATE_FIELD_ID);
                    if (orderDateField && orderDateField.values && orderDateField.values.length > 0) {
                        dealDate = orderDateField.values[0].value.slice(0, 10);
                    } else {
                      dealDate = new Date(deal.created_at * 1000).toISOString().slice(0, 10); // Если нет "Заказ на...", используем дату создания
                    }
                } else {
                    dealDate = new Date(deal.created_at * 1000).toISOString().slice(0, 10); //  Если нет кастомных полей, используем дату создания
                }
        ...
        function showDealsForDate(date, deals) {
            ...
            deals.forEach(deal => {
                ...
                deliveryRangeEl.textContent = `Диапазон доставки: ${getCustomFieldValue(deal, DELIVERY_RANGE_FIELD_ID) || 'Не указано'}`;
                dealEl.appendChild(deliveryRangeEl);

                const exactTimeEl = document.createElement('div');
                exactTimeEl.classList.add('deal-info');
                exactTimeEl.textContent = `К точному времени: ${getCustomFieldValue(deal, EXACT_TIME_FIELD_ID) || 'Не указано'}`;
                dealEl.appendChild(exactTimeEl);

                const addressEl = document.createElement('div');
                addressEl.classList.add('deal-info');
                addressEl.textContent = `Адрес: ${getCustomFieldValue(deal, ADDRESS_FIELD_ID) || 'Не указано'}`;
        ...
        function getCustomFieldValue(deal, fieldId) {
            ...
            return field && field.values && field.values.length > 0 ? field.values[0].value : null;
        }
       ...
       authButton.addEventListener('click', () => {
            window.location.href = `https://aaar31733.amocrm.ru/oauth2/authorize?client_id=6a7f8cd8-d8c3-460c-bcbc-ca4e49a5e496&redirect_uri=https://alerom2006.github.io/Kalendar/oauth_callback.html&response_type=code&state=${widgetInstanceId}`;
        });
        ...
        async function refreshToken() {
          try {
            ...
            const tokenResponse = await fetch('https://aaar31733.amocrm.ru/oauth2/access_token', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
              body: JSON.stringify({
                client_id: '6a7f8cd8-d8c3-460c-bcbc-ca4e49a5e496',
                client_secret: 'fVDHlU3COOt7Zf3jabEwGQPXlUUuaybS49XBiz6TQP4cZTJ0H5F9tjk7uFO4JQFc',
                grant_type: 'refresh_token',
                refresh_token: refreshToken,
                redirect_uri: 'https://alerom2006.github.io/Kalendar/oauth_callback.html',
              }),
            });
            ...
            localStorage.setItem(`amo_access_token_${widgetInstanceId}`, accessToken);
          } catch (error) {
        ...
        clientId = "6a7f8cd8-d8c3-460c-bcbc-ca4e49a5e496";
        clientSecret = "fVDHlU3COOt7Zf3jabEwGQPXlUUuaybS49XBiz6TQP4cZTJ0H5F9tjk7uFO4JQFc";
...
if (typeof window.AmoCRM !== 'undefined') {
  ...
  console.error(`[${widgetInstanceId}] AmoCRM API не найдено до инициализации.`);
}
